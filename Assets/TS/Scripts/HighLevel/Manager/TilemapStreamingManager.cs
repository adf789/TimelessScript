...
public class TilemapStreamingManager : BaseManager<TilemapStreamingManager>
{
    ...
    {
        foreach (var pattern in patternRegistry.AllPatterns)
        {
    ...
    {
        return new Vector2(
            gridOffset.x * pattern.GridSize.x,
            gridOffset.y * pattern.GridSize.y
        );
    }

    private Rect CalculatePatternRect(TilemapPatternData pattern, Vector2Int gridOffset)
    {
        float x = gridOffset.x * pattern.GridSize.x - pattern.GridSize.x * 0.5f;
        float y = gridOffset.y * pattern.GridSize.y - pattern.GridSize.y * 0.5f;

        return new Rect(x, y, pattern.GridSize.x, pattern.GridSize.y);
    }
    ...
    private void DrawLoadedPatternBounds()
    {
        ...
        foreach (var loaded in _loadedPatterns.Values)
        {
            ...
            var size = new Vector3(loaded.PatternData.GridSize.x, loaded.PatternData.GridSize.y, 0);
