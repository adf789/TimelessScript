    ...
    public class TilemapPatternPreview : EditorWindow
    {
        ...
        private void DrawPatternInScene(PreviewPattern preview, bool isSelected)
        {
            ...
            var worldPos = new Vector3(
                _previewOffset.x + preview.GridOffset.x * data.WorldSize.x * _previewScale,
                _previewOffset.y + preview.GridOffset.y * data.WorldSize.y * _previewScale,
                0
            );
            ...
            if (_showGrid)
            {
                ...
                // 가로선
                for (int y = 0; y <= data.GridSize.y; y += 10)
                {
                    Vector3 start = worldPos + new Vector3(0, y * data.TileSize.y * _previewScale, 0);
                    Vector3 end = worldPos + new Vector3(size.x, y * data.TileSize.y * _previewScale, 0);
                    Handles.DrawLine(start, end);
                }
                // 세로선
                for (int x = 0; x <= data.GridSize.x; x += 10)
                {
                    Vector3 start = worldPos + new Vector3(x * data.TileSize.x * _previewScale, 0, 0);
                    Vector3 end = worldPos + new Vector3(x * data.TileSize.x * _previewScale, size.y, 0);
                    Handles.DrawLine(start, end);
                }
            }
